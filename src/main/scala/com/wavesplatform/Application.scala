...
class Application(val actorSystem: ActorSystem, val settings: WavesSettings, ds: DataSource) extends ScorexLogging {
  ...
  private val (history, stateWriter, stateReader, blockchainUpdater, blockchainDebugInfo) = StorageFactory(settings.blockchainSettings, ds).get
...
//    hc.setDriverClassName("org.h2.Driver")
    ...
    RootActorSystem.start("wavesplatform", config) { actorSystem =>
      ...
      new Application(actorSystem, settings, hds) {
